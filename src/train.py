...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            Line embeddings of shape (batch_size, num_lines, embed_dim)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Step 1: Convert scanlines to line embeddings
        # Shape: (batch_size, num_lines, embed_dim)
        x = self.patch_embed(x)

        # Step 2: Prepend CLS token to the sequence
        # CLS token will aggregate information from all scanlines
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)
        x = torch.cat(
            [cls_tokens, x], dim=1
        )  # Shape: (batch, 1+num_lines, embed_dim)
...
def main():
    ...
    model = VisionTransformer(
        ...
        max_lines=128,  # Maximum sequence length (one token per line)
    ).to(device)
